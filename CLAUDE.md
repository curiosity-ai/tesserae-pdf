...
scripts/list-translatable-strings.mjs  the English keys a host has to extract - see "The strings a
                                    host has to extract for us"
```
...
## The strings a host has to extract for us

Everything this package puts on screen goes through **TNT** - `.t()` on a literal, at 82 call sites
across `src/Chrome/` and `src/L10n/` - and reads the very table the host installs with
`TNT.T.SetTranslation`, so a translated key is applied wherever the package uses it. What the
package cannot do is get its keys *into* that table: `tnt extract` scans a repository's own source,
and ours arrives as a NuGet package. So a host that ships twenty languages shows a PDF reader in
English until it repeats those literals in a file of its own, somewhere its extraction looks -
dead code whose only job is to be extracted.

`scripts/list-translatable-strings.mjs` is what writes that list, from the call sites:

```bash
node scripts/list-translatable-strings.mjs                     # the keys
node scripts/list-translatable-strings.mjs --json              # with their call sites
...
Four things about it worth knowing before touching either side:

- **A string with a value in it is `t($"…")`, never `$"…".t()`.** TNT's two entry points differ:
  `t(FormattableString)` looks up the *format* (`"{0} matches"`) and substitutes afterwards, while
  the `.t()` extension takes a `string` - and C# prefers the `string` conversion for an interpolated
  literal, so `$"{_matchTotal} matches".t()` compiled to `TNT.T.t$1(matchTotal + " matches")` and
  asked the table for `"7 matches"`. Eight call sites here did that, silently, and the README's list
  looked right the whole time because it says what the *key* is. They are `t($"…")` now (the
  chrome's partials carry `using static TNT.T;`), which the emitted JS shows as
  `TNT.T.t(FormattableStringFactory.Create("{0} matches", [...]))`. Don't reintroduce the other
  form; the scanner deliberately reads both so a regression shows up as an extracted key rather
  than as a silently English label. Avoid a format specifier inside one as well - `{0:n1}` becomes
  part of the key - and format the value instead.
- **A key is a TNT key, not the source text.** An interpolated `$"Page {page} of {count}"` is looked
  up as `Page {0} of {1}` - what `T.t(FormattableString)` reads off `FormattableString.Format` - so
  that is what the host's file has to carry, as a *plain* literal with `{0}` in it. The scanner does
  that rewrite; a hand-written list gets it wrong.
- **`--update` is what keeps a list from drifting**, and both lists are now generated by it: the
  package README's two tables (`--update Tesserae.Pdf/README.md`) and the host's file. A region is a
  `<tesserae-pdf-strings>` ... `</tesserae-pdf-strings>` pair of comments, in whatever comment syntax
  the file uses, optionally naming an `area` and a `format`. Before the script existed the README's
  chrome table had already drifted by one key (`({0} of {1})`, whose parentheses the toolbar lost),
  which is exactly the kind of error nothing surfaces - a stale key is not a build failure, it is one
  label that stays English.
- **The consumer we know about is mosaik**, whose file is
  `FrontEnd/Mosaik.FrontEnd/src/Helpers/PDF/PdfPackageStrings.cs`; its own CLAUDE.md says to
  regenerate it when the package version moves. Adding or rewording a translatable string here is
  therefore a change with a tail: run the script against the README, and say in the PR that a host's
  extraction file needs regenerating.
