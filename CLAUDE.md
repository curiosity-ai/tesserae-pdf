...
Tesserae.Pdf/                       the package
  ...
  src/L10n/                         PdfL10n + PdfL10nStrings - pdf.js's strings through TNT,
                                    and PdfJs.Translations.cs - the tables a host merges
  .tnt/ + .tnt-content/             TNTC's translation store and the tables the package ships -
                                    see "The package translates itself, with TNTC"
  build/bundle-pdfjs.mjs            esbuild: pdfjs-dist's legacy ESM build -> what we ship
  buildTransitive/*.targets         copies the bundle and the tables into a consuming app's output
...
## The package translates itself, with TNTC

Everything this package puts on screen goes through **TNT** - `.t()` on a literal, or `t($"...")`
where a value goes in - at 82 call sites across `src/Chrome/` and `src/L10n/`, and reads the table
the host installs with `TNT.T.SetTranslation`. For a long time that only meant the strings *could*
be translated: `tnt extract` scans a repository's own sources and ours arrive as a NuGet package, so
every host shipping twenty languages showed an English reader.

The package now translates them itself, with [TNTC](https://www.nuget.org/packages/TNTC), and ships
the result. `Tesserae.Pdf/.tnt/` holds a `translation-<code>.json` per language, `.tnt-content/`
holds the flat pairs an application loads, and both are committed.

**The loop is the `tntc-translate` skill in `.claude/skills/`** (installed by
`tntc install-skill .`, and kept up to date by the tool - commit the diff when it says so). In
short, run from the repository root:

```bash
dotnet tool install --global TNTC
export DOTNET_ROLL_FORWARD=LatestMajor          # only .NET 10 is installed here; TNTC targets 9

tntc extract Tesserae.Pdf                       # after adding or rewording a string
tntc missing Tesserae.Pdf --limit 50 --output batch.json
#   fill every empty value in batch.json - you are the translator, the tool never calls a model
tntc apply   Tesserae.Pdf batch.json --model <the model id you are running as>
tntc verify  Tesserae.Pdf                       # exits 3 on any error; run it last
```

Four things about the whole arrangement worth knowing before touching either half:
...
- **A string with a value in it is `t($"…")`, never `$"…".t()`.** TNT's two entry points differ:
  ...
  asked the table for `"7 matches"`. Eight call sites here did that, silently. They are `t($"…")`
  now (the chrome's partials carry `using static TNT.T;`), which the emitted JS shows as
  `TNT.T.t(FormattableStringFactory.Create("{0} matches", [...]))`. Avoid a format specifier inside
  one as well - `{0:n1}` becomes part of the key - and format the value instead.
- **The tables ship as assets, renamed to `.json` on the way.** The csproj packs `.tnt-content/*.tnt`
  under `l10n/`, and `Tesserae.Pdf.targets` copies them into the consumer's output beside pdf.js as
  `assets/js/pdf/l10n/<code>.json` - resolved the same two ways as pdf.js itself (`../l10n` inside a
  restored package, `.tnt-content` in this repo), so the sample exercises the copy a real consumer
  gets. The rename is not cosmetic: `.tnt` is a content type ASP.NET Core's static-file middleware
  does not know, and an unknown one is answered with 404 rather than the file - the reader would fall
  back to English with nothing in the console to say why. `PdfJs.LoadTranslationsAsync` reads them.
...
- **A missing table is a fallback, never an error.** An older package, a language nobody translated,
  ...
  which it forgets rather than caches, so a viewer opened after the network came back is translated.
